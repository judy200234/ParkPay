...
public class EncryptorGUICheckOut extends Thread implements ActionListener{
	JFrame frame;
	...
	public void runMain(){
		...
		frame = new JFrame("Parking Check Out Screen");//視窗名稱
	...
	public void actionPerformed(ActionEvent k) {
	  ...
	  jdbc123CheckOut jdbc = new jdbc123CheckOut();
	  
	  jdbc.jdbc(val);
	  
	  Credit = jdbc.getCredit();
	  discountTime = jdbc.getShowDiscountTime();
	  whichmachine = jdbc.getWhichMachine();
	  showsec = jdbc.getShowsec();
	  
	  label5.setText("$" + jdbc.getParkingFee());
	  label6.setText("$" + Credit);
	  if(whichmachine==2){
		  if(showsec<60){
			  TextArea1.setText("Start: " + jdbc.getstarttime() + "\n" + "End : " + jdbc.getendtime() + "\n" + "Total: " + showsec + " sec" + "\n" + "Discount Time :" + discountTime + " mins");
		  }else if((60<showsec)&&(showsec<3600)){
			  TextArea1.setText("Start: " + jdbc.getstarttime() + "\n" + "End : " + jdbc.getendtime() + "\n" + "Total: " + (showsec/60) + " min " + (showsec-((showsec/60)*60)) + " sec" + "\n" + "Discount Time :" + discountTime + " mins");
		  }else if(showsec>3600){
			  TextArea1.setText("Start: " + jdbc.getstarttime() + "\n" + "End : " + jdbc.getendtime() + "\n" + "Total: " + (showsec/3600) + " hr " + ((showsec-((showsec/3600)*3600))/60) + "min " + (showsec-(((showsec-((showsec/3600)*3600))/60)*60)-((showsec/3600)*3600)) + " sec" + "\n" + "Discount Time :" + discountTime + " mins");
		  }
	  }
	  else{
		  TextArea1.setText("Error! Double Check Out");
	  }
